...
for f in cat_feats:
    ...
    if f in studentVle_df.reset_index(): vals += studentVle_df.reset_index()[f].dropna().tolist()
    cat_encoders[f].fit(vals)

for f in num_feats:
    vals = []
    for df in [merged, assessments_df, studentAssessment_df.reset_index(), studentVle_df.reset_index()]:
