...
from models import AttentionPolicyNetwork_gated as AttentionPolicyNetwork, sample_action, select_from_action, create_mil_model_with_dict
from utils import (
    get_data_directory,
    get_model_name,
...
def create_rl_model(args, mil_best_model_dir):
    ...
    task_model = load_mil_model_from_config(os.path.join(mil_best_model_dir, "..", "best_model_config.json"),
                                            state_dict)
    # Instantiate AttentionPolicyNetwork
    policy_network = AttentionPolicyNetwork(
        task_model=task_model,
        state_dim=args.state_dim,
        hdim=args.hdim,
        k=args.bag_size,
        gated_temperature=args.gated_temperature,
        ...
        linear_attention_only=args.linear_attention_only,
        gated_attention_size=args.gated_attention_size,
        gated_attention_dropout_p=args.gated_attention_dropout_p,
...
def load_model_from_config(mil_config_file, rl_config_file, rl_model_file):
    ...
    gated_temperature=rl_config.get('gated_temperature'),
    learning_rate= 0,
    device=device,
    task_type=rl_config.get('task_type', 'classification'),
    min_clip=rl_config.get('min_clip', None),
    max_clip=rl_config.get('max_clip', None),
    no_autoencoder=rl_config.get('no_autoencoder_for_rl', False),
    linear_attention_only=rl_config.get('linear_attention_only', False),
    gated_attention_size=rl_config.get('gated_attention_size'),
    gated_attention_dropout_p=rl_config.get('gated_attention_dropout_p'),
)
            ...
            for i in range(batch_x_embeddings.shape[0]): # Iterate through bags in the batch
                ...
                for j in range(num_padded_instances): 
                    ...
                    instance_data = {
                        ...
                        "predicted_bag_label_gated": pred_bag_label_val,
...
def train(
    ...
    output_file = os.path.join(run_dir, "attention_linear_outputs.csv")
...
def main_sweep():
    ...
    [
        {"params": policy_network.attn_layer.parameters(),  "lr": args.actor_learning_rate},
