...
#SBATCH --output=../logs/DATASET_NAME/multi_head_attention/seed_0/%j.out
#SBATCH --error=../logs/DATASET_NAME/multi_head_attention/seed_0/%j.err
...
prefix="loss_attention_multi_head"
  ...
  for bag_size in "${bag_sizes[@]}"; do
    for embedding_model in "${embedding_models[@]}"; do
      for baseline_type in "${baseline_types[@]}"; do
        ...
        CUDA_VISIBLE_DEVICES=0 python multi_head_attention.py \
          --rl \
          --gpu 0 \
