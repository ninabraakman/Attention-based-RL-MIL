...
baselines=("MeanMLP" "MaxMLP" "AttentionMLP" "repset" "SimpleMLP")
...
for dataset in "${datasets[@]}"; do
    if [ "$dataset" = "political_data_with_age" ]; then
        ...
        for label in "${labels[@]}"; do
            for embedding_model in "${embedding_models[@]}"; do
                for random_seed_index in "${!random_seeds[@]}"; do
                    ...
                    SESSION_NAME="${random_seed}_${dataset}_${baseline}_${label}"
                    echo $SESSION_NAME
                    screen -dmS "$SESSION_NAME" bash -c "
                    CUDA_VISIBLE_DEVICES=$gpu python3 multiple_run_mil.py \
                                                    --dataset "$dataset" \
                                                    --baseline "$baseline" \
                                                    --label "$label" \
