...
#SBATCH --output=../logs/DATASET_NAME/gated_attention/seed_0/%j.out
#SBATCH --error=../logs/DATASETNAME/gated_attention/seed_0/%j.err
...
prefix="loss_attention_linear"
rl_model="policy_only"
reg_alg="sum"
# When using this you are running linear attention, not gated attention. 
# Keep in mind to change the prefix, attention output file name, and have a yaml config file with the right prefix ready.
# linear_attention_only="--linear_attention_only"     
  ...
  for bag_size in "${bag_sizes[@]}"; do
    for embedding_model in "${embedding_models[@]}"; do
      for baseline_type in "${baseline_types[@]}"; do
        ...
        CUDA_VISIBLE_DEVICES=0 python gated_attention.py \
          --rl \
          --gpu 0 \
          ...
          --reg_alg $reg_alg \
          # $linear_attention_only          # Only use when you want to run linear attention
